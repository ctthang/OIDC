...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        options.Events = new JwtBearerEvents
        {
            ...
            {
                // Check cnf claim and client certificate header
                var cnf = context.Principal?.FindFirst("cnf")?.Value;
                if (!string.IsNullOrEmpty(cnf))
                {
                    var httpContext = context.HttpContext;
                    var clientCertHeader = httpContext.Request.Headers["X-Client-Cert"].FirstOrDefault();
                    if (string.IsNullOrEmpty(clientCertHeader))
                    {
                        context.Fail("Missing client certificate header.");
                    ...
                    {
                        var certFromHeader = new X509Certificate2(Convert.FromBase64String(clientCertHeader));
                        var thumbprint = Base64UrlEncoder.Encode(certFromHeader.GetCertHash(System.Security.Cryptography.HashAlgorithmName.SHA256));
                        if (!string.Equals(thumbprint, thumbprintElement.GetString(), StringComparison.OrdinalIgnoreCase))
                        {
